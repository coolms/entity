...
All notable changes to `coolms/entity` are recorded here.
...
moved here: `EntityResolverChainTest`. Nothing under `src/` changes.
...
```
composer require coolms/entity:^2.0
```
...
coolms/entity:^2.0@alpha` admits the alpha of the package it names and
...
A bare `composer require coolms/entity` resolves **successfully** to v1.0.0 --
...
This release moves `coolms/entity` to `2.0.0` **without a single change to its
...
First release. Entity contracts for the platform: the extras engine for dynamic
per-entity fields and its ports, the alias registry, entity resolvers, virtual
fields, and the shared provider traits.
